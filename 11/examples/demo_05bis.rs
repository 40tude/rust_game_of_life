// cargo run -p step_11 --release --example demo_05bis
//
//! GPU Benchmark - Uses shaders
// Version 05bis = Version 05 + async IO
...
impl App {
    fn new(log_sender: mpsc::Sender<String>) -> Self {
        Self {
            window: None,
            pixels: None,
            uniform_buffer: None,
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        let window = event_loop
            .create_window(Window::default_attributes().with_title("Step 11: GPU bench 02 + async IO").with_resizable(false))
            .unwrap();
        ...
        // Asynchronously send the presentation mode to the console
        let present_mode = format!("Present mode: {:?}", pixels.present_mode());
        let _ = self.log_sender.send(present_mode);
            ...
            WindowEvent::RedrawRequested => {
                ...
                if self.last_print.elapsed() >= Duration::from_secs(1) {
                    if let Some(start) = self.start_time {
                        ...
                        // Asynchronously send the FPS to the console
                        let fps_msg = format!("[{:2}s] Frames: {} | FPS: {:.0}", elapsed, self.frame_count, fps);
                        let _ = self.log_sender.send(fps_msg);
                    }
...
fn main() -> Result<()> {
    ...
    // Create a channel for async writings
    let (log_sender, log_receiver) = mpsc::channel::<String>();

    // Start a dedicated thread to write to the console
    thread::spawn(move || {
    ...
    let mut app = App::new(log_sender);
