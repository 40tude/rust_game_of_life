// cargo run -p step_11 --example demo_03bis
//
//!GPU testing - FPS compare
// Version 03bis = Version 03 + async IO
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        let window = event_loop
            .create_window(Window::default_attributes().with_title("Step 11: GPU bench 00 + async IO").with_resizable(false))
            .unwrap();
            ...
            WindowEvent::RedrawRequested => {
                if let Some(pixels) = &mut self.pixels {
                    ...
                    if elapsed >= Duration::from_secs(1) {
                        ...
                        let fps_msg = format!("FPS (Immediate): {:.1}", fps);
