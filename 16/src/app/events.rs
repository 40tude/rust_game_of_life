    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _: window::WindowId, event: WindowEvent) {
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                // match life::step_life(&self.board_current, &mut self.board_next, self.board_width, self.board_height) {
                //     Ok(_) => {
                //         // Swap boards only if step_life succeeded
                //         std::mem::swap(&mut self.board_current, &mut self.board_next);
                //     }
                //     Err(e) => {
                //         let error_msg = format!("Error updating game state: {}", e);
                //         error!("{}", error_msg);
                //         self.set_error(error_msg, 5);
                //         // Don't swap boards - keep the current valid state
                //     }
                // }
