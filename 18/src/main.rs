...
use step_18::{Result, app::state::App};
...
fn setup_logging() -> Result<()> {
    Logger::try_with_str("step_18=info, wgpu_core=info, wgpu_hal=warn, wgpu=off, naga=off")?.start()?; // console only 
...
fn handle_parameters() -> Result<PathBuf> {
    ...
    let path_to_pattern = match matches.get_one::<PathBuf>("pattern") {
        ...
        None => {
            ...
            let path = PathBuf::from(step_18::config::DEFAULT_PATTERN_PATH);
            if !is_valid_file_path(&path) {
