...
use step_14::{Result, app::state::App};
...
fn setup_logging() -> Result<()> {
    ...
    Logger::try_with_str("step_14=trace, wgpu_core=info, wgpu_hal=warn, wgpu=off, naga=off")?
        ...
        // .write_mode(WriteMode::Async) // async does NOT work under WIN 11. https://docs.rs/flexi_logger/latest/flexi_logger/error_info/index.html
...
fn handle_parameters() -> Result<PathBuf> {
    ...
    let path_to_pattern = match matches.get_one::<PathBuf>("pattern") {
        ...
        None => {
            ...
            let path = PathBuf::from(step_14::config::DEFAULT_PATTERN_PATH);
            if !is_valid_file_path(&path) {
